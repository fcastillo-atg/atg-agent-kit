...
**Side effects:** Never commits, never posts to Jira, never runs Gradle, never creates or updates PRs. **Does** write (or overwrite) `EXPLAIN.md` under the ticket’s `bin/stories/` directory when that story folder already exists. Chat still gets the full brief.
...
- Existing `EXPLAIN.md` (optional) — refresh/overwrite; do not merge blindly with stale content
...
If story files or Jira are missing, continue from the diff alone. Optionally suggest `/atg:brief {TICKET}` only as a follow-up — do not require it. **Do not create** a new `bin/stories/...` folder from this command.
...
### Step 6: Persist `EXPLAIN.md` (default)

When a story directory already exists at `bin/stories/{year}/{month}/{TICKET}-{slug}/` (service or monorepo path), **write or overwrite**:

`bin/stories/{year}/{month}/{TICKET}-{slug}/EXPLAIN.md`

File contents:

1. A short header block:
   - `# {TICKET} — Plain-language change brief`
   - `**Generated by:** `/atg:explain``
   - `**Date:**` today
   - `**Branch:**` current branch (or PR head)
   - `**Diff base:**` `origin/main...HEAD` or `PR #{N}`
   - `**PR:**` URL or `none open`
2. Then the same fixed-shape brief from Step 5 (Verdict through Merge confidence), plus the Next Steps footer from Step 7.

Rules:

- **Overwrite** a prior `EXPLAIN.md` with a fresh brief (do not append).
- If **no** story directory exists: chat-only; say so once and optionally suggest `/atg:brief {TICKET}` — do **not** invent a `bin/stories/` path.
- If the user keeps a second local checkout of the same story folder (e.g. main vs worktree), sync `EXPLAIN.md` there only when that folder already exists and the team has been keeping those copies in sync; otherwise write the checkout you are working in.
- Mention the written path in the chat reply (one line is enough).

### Step 7: Next Steps footer
...
Also writes `bin/stories/.../WBPR-4595-.../EXPLAIN.md` with the same body plus the header metadata block.
...
| No story dir + no Jira | Explain from diff + commits + changeset only (chat); skip file write |
| Story dir missing | Chat brief only; do not create `bin/stories/` from this command |
...
- Default **does** persist `EXPLAIN.md` under an existing story dir; still never commits that file.
