...
If your PR changes files under **`wavebid-a2o-service/`** or **`wavebid-a2o-ui/`**, CI (`.github/workflows/changeset-check.yml`) expects a **`.changeset/*.md`** file on the branch, unless the PR has the **`skip-changelog`** label.
...
- **Full procedure:** Read `.cursor/commands/gsd/changeset-wavebid-a2o.md` from the monorepo root.

## Product copy

See `.changeset/README.md` for writing style (user-facing sentences, not ticket IDs only).
